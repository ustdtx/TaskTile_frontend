...
import ManagerTaskDetailsModal from "../../../../components/ManagerTaskDetailsModal";
...
export default function TasksPage() {
  ...
  const [tasks, setTasks] = useState<{ id: string; title: string; description: string; deadline: string; status: string; assigneeName: string }[]>([]);
      ...
      try {
        ...
        if (Array.isArray(data)) {
          const mappedTasks = data.map((task: any) => {
            const assignee = members.find((member) => member.id === task.assigneeId); // Match assigneeId with members
            return {
              id: task.id,
              title: task.title,
              ...
              assigneeName: assignee ? assignee.username : "Unassigned", // Use username if found, otherwise "Unassigned"
            };
          });
  ...
  return (
    <Container>
      ...
      {
      selectedTask && <ManagerTaskDetailsModal task={selectedTask} onClose={() => setSelectedTask(null)} members={members} userId={userId || ""} />}
    </Container>
  );
}